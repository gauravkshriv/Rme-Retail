...
@Component({
  selector: 'app-commercialview',
  templateUrl: './commercialview.component.html',
  styleUrls: ['./commercialview.component.scss']
})
 ...
 carouselItems =[];
  ...
  ngOnInit() {
   this.showcontacts = this.utilservice.routedsellproperty;   
    ...
    //////////////////////////
    this.commecialSinglePage(this.pid)
  ...
  commecialSinglePage(pid)
  {
    this.isLoading = true;
   this.myUserSub =  this.apiservice.getCommecialSinglePage(pid)
    //  this.http.get('http://ec2-3-80-207-138.compute-1.amazonaws.com/product/prebuilt?pid='+this.pid)
     .subscribe((data:any)=>
     {
      ...
      this.isLoading = false;
       this.singleview = data.extraData.commercialAsset;

       console.log('=>',this.singleview);
       
       
       let homappl = this.singleview.commercialStructuralDocument.basicUtilities;
      
       let furnit = this.singleview.commercialStructuralDocument.furniture;
   ...
   console.log('fun',fun,i,furnit);
    ...
    if(this.singleview.nearByLocation.OTHER)
     {
      ...
      // var tempArray=[];
      // this.singleview.commercialStructuralDocument.floorAndCeiling.ceiling.forEach(v => {
      //   tempArray.push(this.enumservice.kytname.get(v))
      //   // console.log("v",v);
      // });
      // this.singleview.commercialStructuralDocument.floorAndCeiling.ceiling = tempArray;

      // var tempArray=[];
      // this.singleview.commercialStructuralDocument.floorAndCeiling.flooring.forEach(v => {
      //   tempArray.push(this.enumservice.kytname.get(v))
      //   // console.log("v",v);
      // });
      // this.singleview.commercialStructuralDocument.floorAndCeiling.flooring = tempArray;
    ...
    this.singleview.commercialStructuralDocument.propertyFacing = this.enumservice.kytname.get(this.singleview.commercialStructuralDocument.propertyFacing);
     ...
     PropertyView()
     {
      this.GetIpAddress((err,res)=>{
         ...
         let body = {
          ...
          {
           await this.commecialSinglePage(this.pid);
          console.log("check view data",data);
          },
          (err) => {
            console.log("got error", err)
